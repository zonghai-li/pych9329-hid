...
def test_modifier_only_and_release(fake_transport):
    """
    ...
    gui.keyDown('shift')
    assert fake_transport._hid.keyboard_reports, "modifier down should send a report"
...
def test_many_keys_truncated_to_six(fake_transport):
    """
    ...
    _, codes = fake_transport._hid.keyboard_reports[-1]
    assert len(codes) <= 6


def test_hotkey_includes_shift_mod_from_char(fake_transport):
    """
    ...
    assert any(r[0] != 0 for r in fake_transport._hid.keyboard_reports), "shift mod from '@' should be applied"
