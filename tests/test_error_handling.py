...
from conftest import FakeCH, FakeTransport, FakeTransportWithFakeCH, build_ack
...
class TestCH9329ErrorHandling:
    ...
    def test_timeout_on_no_response(self, fake_transport):
        """Test that timeout returns None and issues warning when device doesn't respond."""
        import warnings
...
class TestHIDControllerErrorHandling:
    ...
    def test_invalid_modifier_key(self, fake_ch_transport):
        """Test handling of invalid modifier key."""
        gui = HIDController(fake_ch_transport)
        fake_ch_transport._hid.keyboard_reports.clear()
        
        gui.keyDown('invalid_modifier')
        
        # Should handle gracefully without crashing
        assert isinstance(fake_ch_transport._hid.keyboard_reports, list)
    
    def test_empty_string_write(self, fake_ch_transport):
    ...
    def test_zero_duration_movement(self, fake_ch_transport):
        """Test movement with zero duration."""
        gui = HIDController(fake_ch_transport, screen_width=800, screen_height=600)
        fake_ch_transport._hid.mouse_calls.clear()
        
        gui.moveTo(100, 100, duration=0)
        
        assert len(fake_ch_transport._hid.mouse_calls) >= 1
    
    def test_negative_duration_movement(self, fake_ch_transport):
        ...
        # Should handle gracefully
        assert isinstance(fake_ch_transport._hid.mouse_calls, list)
    
    def test_zero_scroll(self, fake_ch_transport):
    ...
    def test_large_scroll_value(self, fake_ch_transport):
    ...
    def test_invalid_mouse_button(self, fake_ch_transport):
    ...
    def test_release_non_pressed_key(self, fake_ch_transport):
    ...
    def test_hotkey_with_empty_keys(self, fake_ch_transport):
    ...
    def test_out_of_bounds_coordinates_positive(self, fake_ch_transport):
        """Test moving to coordinates beyond screen bounds (positive)."""
        gui = HIDController(fake_ch_transport, screen_width=1920, screen_height=1080)
        fake_ch_transport._hid.mouse_calls.clear()
    ...
    def test_out_of_bounds_coordinates_negative(self, fake_ch_transport):
    ...
    def test_click_with_zero_clicks(self, fake_ch_transport):
    ...
    def test_drag_with_zero_distance(self, fake_ch_transport):
...
class TestBoundaryConditions:
    ...
    def test_six_keys_pressed(self, fake_ch_transport):
        ...
        _, codes = fake_ch_transport._hid.keyboard_reports[-1]
        assert len(codes) == 6
    
    def test_seven_keys_pressed(self, fake_ch_transport):
    ...
    def test_all_modifiers_pressed(self, fake_ch_transport):
        ...
        mod, _ = fake_ch_transport._hid.keyboard_reports[-1]
        assert mod == 0x0F  # All modifiers set
    
    def test_all_mouse_buttons_pressed(self, fake_ch_transport):
        ...
        _, _, _, buttons, _ = fake_ch_transport._hid.mouse_calls[-1]
        assert buttons == 0x07  # All buttons set
    
    def test_rapid_key_presses(self, fake_ch_transport):
        ...
        assert len(fake_ch_transport._hid.keyboard_reports) >= 20  # At least down+up for each
    
    def test_rapid_mouse_movements(self, fake_ch_transport):
        ...
        assert len(fake_ch_transport._hid.mouse_calls) >= 10
